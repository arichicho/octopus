...
async function getCalendarEvents(uid: string, dateISO: string): Promise<ContextEvent[]> {
  ...
  return json.items
    ...
    .map((e: any) => {
      ...
      if (e.start?.dateTime) {
        // Event with specific time - keep as is (already in UTC from Google Calendar API)
        start = e.start.dateTime;
      } else if (e.start?.date) {
        // All-day event - set to working hours in UTC
...
async function getGmailThreads(uid: string, lookbackDays: number): Promise<ContextEmailThread[]> {
  ...
  // For each thread, fetch details to get snippet and historyId
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Build context pack with real integrations if available; tolerate partial data
    const tasks = await getTasksForUser(auth.uid, auth.email || null);
    const events = await getCalendarEvents(auth.uid, date).catch(() => []);
    const emailThreads = await getGmailThreads(auth.uid, DEFAULT_MY_DAY_SETTINGS.privacy.maxEmailLookbackDays).catch(() => []);
    const docs = await getDriveDocs(auth.uid).catch(() => []);

    const peopleIndex: PeopleIndex = {}; // can be filled later linking with CRM/people collection

    const context: ContextPack = {
      dateISO: date,
      settings: DEFAULT_MY_DAY_SETTINGS,
      events,
      tasks,
      emailThreads,
      docs,
